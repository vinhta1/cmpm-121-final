...
export class Grid {
    ...
    public setCell(GridCell) {
        const index = (GridCell.y * this.width + GridCell.x)*OFFSET;
        if (this.viewer[index] != GridCell.x || this.viewer[index + 1] != GridCell.y) {
            throw new Error(`Buffer isn't lined up: input ${GridCell.x},${GridCell.y}, buffer ${this.viewer[index]},${this.viewer[index+1]}`);
        }
    ...
    public getCell(x: number, y: number): GridCell {
        ...
        if (this.viewer[index] != x || this.viewer[index + 1] != y) {
            throw new Error(`Buffer isn't lined up: input ${x},${y}, buffer ${this.viewer[index]},${this.viewer[index+1]}`);
        }
        return {
            x:this.viewer[index],

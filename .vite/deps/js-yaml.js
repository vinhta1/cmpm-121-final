...
// node_modules/.deno/js-yaml@4.1.0/node_modules/js-yaml/dist/js-yaml.mjs
...
function storeMappingPair(
  ...
  if (keyTag === "tag:yaml.org,2002:merge") {
    ...
    if (
      !state.json && !_hasOwnProperty$1.call(overridableKeys, keyNode) &&
...
function testDocumentSeparator(state) {
  ...
  if (
    (ch === 45 || ch === 46) && ch === state.input.charCodeAt(_position + 1) &&
...
function readDocument(state) {
  ...
  skipSeparationSpace(state, true, -1);
  if (
    state.lineIndent === 0 && state.input.charCodeAt(state.position) === 45 &&
...
function isPrintable(c) {
  return 32 <= c && c <= 126 ||
    ...
    57344 <= c && c <= 65533 && c !== CHAR_BOM || 65536 <= c && c <= 1114111;
}
function isNsCharOrWhitespace(c) {
  return isPrintable(c) && c !== CHAR_BOM && c !== CHAR_CARRIAGE_RETURN &&
    c !== CHAR_LINE_FEED;
}
...
function isPlainSafeFirst(c) {
  return isPrintable(c) && c !== CHAR_BOM && !isWhitespace(c) &&
    ...
    c !== CHAR_VERTICAL_LINE && c !== CHAR_EQUALS && c !== CHAR_GREATER_THAN &&
    c !== CHAR_SINGLE_QUOTE && c !== CHAR_DOUBLE_QUOTE && c !== CHAR_PERCENT &&
    ...
    if (
      ...
      if (type2.represent) {
        ...
        if (_toString.call(type2.represent) === "[object Function]") {
          ...
          throw new exception(
            "!<" + type2.tag + '> tag resolver accepts not "' + style +
              '" style',
          );
        }
        state.dump = _result;
...
function renamed(from, to) {
  return function () {
    throw new Error(
      "Function yaml." + from + " is removed in js-yaml 4. Use yaml." + to +
